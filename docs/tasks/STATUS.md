...
3. **Each parser tags its emitted events** with the right `ActivityCategory` — added incrementally as each parser is touched (the parser knows its output's category; the *vocabulary* is forensicnomicon's, re-exported as `issen_core::ActivityCategory`). A formal forensicnomicon source→category lookup table (1b) is deferred until ≥2 parsers need to share one.
   ...
   - ✅ **Mixed parsers — per-event-type mapping (3/5):** `boot_log` (ld.preload→Persistence / sshd→SystemState, `22ff1b8`/`7921011`), `evtx` (`event_id_to_category` incl. 1102→AntiForensics, `3902f25`/`44926f9`), `srum` (network→NetworkActivity / app→Execution, real SRUDB, `5945d01`/`fc92463`). ⬜ blocked: `lxss` (no WSL hive fixture anywhere), `regcatalog` (genuinely needs the deferred 1b forensicnomicon per-descriptor table — it surfaces the whole catalog).
   - 🚩 **Re-diagnosis of the 4 "blocked" registry parsers (background agent, 2026-06-18) — 2 are PARSER BUGS, not data gaps** (verified against the real Case-001 hives with python-registry): **`svcdiff`** reads `CurrentControlSet\Services` — a *volatile symlink that does not exist in any offline hive* (offline has `ControlSet001`, per `Select\Current`); it returns zero on EVERY offline image → fix `winreg-artifacts::svc_diff` to resolve `Select\Current`→`ControlSet00N` (then Case-001 SYSTEM = 453 services). **`comhijack`** reads `NTUSER.DAT` for `Software\Classes\CLSID`, but on Win10 that lives in **`UsrClass.dat`** → fix to accept UsrClass.dat (+ SOFTWARE HKCR). **`lsadump`** DCC2 `Cache` genuinely empty in Case-001 (correct zero), but `Policy\Secrets` IS present (`$MACHINE.ACC`/`DPAPI_SYSTEM`/`NL$KM`) — a real-data test is possible if the event path surfaces secrets. **`amcache`** no `Amcache.hve` in `tests/data/` but extractable from our E01s (`Windows/AppCompat/Programs/Amcache.hve`) or Ali Hadi Challenge #7's small hive. NOTE: `case001-hives/` is the **DC (CITADEL-DC01)**; the Desktop workstation hives (richer) are in `DESKTOP-SDN1RPT-Protected Files.zip`.
