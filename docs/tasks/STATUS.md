...
3. **Each parser tags its emitted events** with the right `ActivityCategory` — added incrementally as each parser is touched (the parser knows its output's category; the *vocabulary* is forensicnomicon's). A formal forensicnomicon source→category lookup table (1b) is deferred until ≥2 parsers need to share one. **NOTE:** several parser crates are currently dirty from another session (`issen-parser-linux/boot_log.rs`, `issen-mem`, `issen-signatures`) — tag clean parsers first.
