...
use issen_signatures::heuristics::{self, HeuristicsConfig};
...
fn main() -> Result<()> {
    ...
    if let Some(ref mirror_path) = sources.mft_mirror {
        match issen_mft_tree::mirror::validate_mirror(&sources.mft, mirror_path) {
            Ok(result) => {
                if result.is_valid() {
    ...
    if let Some(ref logfile_path) = sources.logfile {
        match issen_mft_tree::logfile::validate_logfile(logfile_path) {
            Ok(validation) => {
...
fn try_open_collection(
    ...
    Option<(
        ...
        Option<App>,
        issen_unpack::CollectionManifest,
    )>,
> {
    use issen_unpack::registry::open_collection;
...
fn run_workbench(
    ...
    mft_app: Option<App>,
    _manifest: issen_unpack::CollectionManifest,
) -> Result<()> {
