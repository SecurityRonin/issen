//! Google Drive OAuth2 auth for RapidTriage.
//!
//! Auth priority (highest → lowest):
//!   1. Service account JSON  — env `GOOGLE_APPLICATION_CREDENTIALS`
//!   2. Stored user OAuth token — `~/.config/rapidtriage/gdrive_token.json`
//!   3. Public (unauthenticated) — share-link files only
//!
//! Browser flow:
//!   `rt gdrive auth login`
//!
//! Embedded OAuth credentials are registered under the RapidTriage GCP project
...
/// Embedded OAuth client ID (RapidTriage Desktop app, GCP project rapidtriage).
...
/// How RapidTriage will authenticate to Google Drive for a given operation.
#[derive(Debug)]
...
///
/// Returns `~/.config/rapidtriage/gdrive_token.json` on Unix,
/// or `%APPDATA%\rapidtriage\gdrive_token.json` on Windows.
pub fn token_cache_path() -> PathBuf {
    ...
    base.join("rapidtriage").join("gdrive_token.json")
}
...
pub fn initiate_browser_auth() -> anyhow::Result<String> {
    ...
    eprintln!("\nOpen this URL to authorise RapidTriage to access Google Drive:");
    eprintln!("\n  {auth_url}\n");
...
fn read_code_from_callback(stream: TcpStream) -> anyhow::Result<String> {
    ...
    let html = b"<html><body><h1>RapidTriage: authorised.</h1>\
...
mod tests {
    ...
    #[test]
    fn token_cache_path_contains_rapidtriage() {
        let path = token_cache_path();
        let s = path.to_string_lossy();
        assert!(
            s.contains("rapidtriage"),
            "token cache path should contain 'rapidtriage', got: {s}"
        );
    }
