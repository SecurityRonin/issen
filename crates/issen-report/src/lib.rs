...
fn key_judgment(correlations: &[Correlation]) -> String {
    if correlations.is_empty() {
        return "No cross-artifact correlations were produced for this case. The \
                appendix lists the individual scan findings."
            .to_string();
    }
...
.brand-ronin {{ font-size: 0.74rem; color: #8899aa; }}
.brand-ronin a, footer a {{ color: var(--link); text-decoration: none; }}
.brand-ronin a:hover, footer a:hover {{ text-decoration: underline; }}
...
<div class="meta">"#,
    ...
    let _ = write!(
        ...
        r#"</div>
<footer>
Generated by Issen &middot; 浪人一閃 Ronin Issen &middot; a <a href="https://www.securityronin.com/" target="_blank" rel="noopener">Security Ronin</a> product &middot; {generated}<br>
Findings are observations (consistent with), never verdicts.
</footer>
"#,
...
fn render_correlated_findings(html: &mut String, data: &ReportData) {
    ...
    for group in group_rules(&data.correlations) {
        ...
        for inst in group.instances.iter().take(MAX_INSTANCES) {
            // Collapse a degenerate window: when the two ends render to the same
            // (second-precision) instant, show one time, not "X — X".
            let first_s = format_ns(inst.first_ts);
            let last_s = format_ns(inst.last_ts);
            let window = if first_s == last_s {
                html_escape(&first_s)
            } else {
                format!("{} &mdash; {}", html_escape(&first_s), html_escape(&last_s))
            };
            let _ = writeln!(
                html,
                "<details class=\"instance\"><summary>{window} &middot; scope: {scope}</summary>",
