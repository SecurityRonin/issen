...
impl Qcow2DataSource {
    ...
    pub fn open_zip(zip_path: &Path) -> Result<Self, Qcow2Error> {
        ...
        let backing = issen_unpack::backing::archive_backing(zip_path, &plan, &["qcow2", "qcow"])
            .map_err(|e| Qcow2Error::Qcow2(format!("open_zip: {e}")))?;
        let reader = qcow2::Qcow2Reader::open_reader(Box::new(backing))?;
        let size = reader.virtual_disk_size();
        Ok(Self {
            reader: Mutex::new(reader),
            size,
    ...
    fn read_at(&self, offset: u64, buf: &mut [u8]) -> Result<usize, RtError> {
        ...
        let mut guard = self.reader.lock().map_err(|_| {
            RtError::Io(std::io::Error::other(
                "Qcow2DataSource: reader mutex poisoned",
            ))
        })?;
