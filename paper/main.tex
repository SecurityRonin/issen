...
$f_{\text{parse}}: B^{*} \to R$ is a parser valued in records. We name two
architectural principles that follow from this definition: \emph{Parser
Independence} (identical bytes yield identical records under a fixed
deterministic parser, regardless of navigation primitive) and the
\emph{Layer Dependency Rule} (a parser that imports a NAVIGATE component
cannot be source-agnostic); the second is the contrapositive of the first,
and both are enforced mechanically by Issen's crate boundaries. We define \emph{attacker-durability} of captured evidence and
