...
\smallskip\noindent\textbf{On the nature of the results below.}
The propositions in Sections~\ref{sec:properties}A and~\ref{sec:properties}B
are \emph{structural}: they follow from the type-theoretic definitions of
navigation primitive and parser rather than from non-trivial mathematical
analysis. Their value is not mathematical depth but architectural
\emph{explicitness}. Naming them converts implicit engineering folk wisdom
into falsifiable claims: the Parser Independence proposition becomes an
empirically testable prediction (Section~\ref{sec:case3}); the Layer
Dependency Rule becomes an architectural constraint admissible to automated
enforcement. The attacker-durability results (Section~\ref{sec:properties}C)
are genuine conditional propositions whose hypotheses carry real content.
...
the semantic content and is the load-bearing claim of the proposition: it says
...
prediction. We note that both halves of the proof are short substitutions
from the definitions; the value of the proposition is in making the
principle explicit and testable, not in the mathematical derivation.

\paragraph{Scope of the determinism assumption.} The proposition requires the
...
\begin{proof}
Proposition~\ref{thm:parser-indep} states that semantic invariance holds for
...
function on $\Bstar$, and Proposition~\ref{thm:parser-indep} does not apply to
...
well-typed. Hence $p$ fails Type composition (Proposition~\ref{thm:parser-indep}.I)
