...
This section delivers Contributions~2,~3, and~4. We establish a Parser
Independence proposition, derive the layer dependency rule as its
contrapositive, formalize attacker-durability, characterize the distinct
durability profiles of \src{Q} and \src{C}, and give a schema-conscious
definition of hash-pivot.

\smallskip\noindent\textbf{Nature of the results.}
Parser Independence and Layer Dependency are \emph{architectural principles}
stated in formal language so they can be named, referenced, and empirically
tested. The attacker-durability propositions (Section~\ref{sec:attacker-durability})
are genuine conditional results whose adversary-model hypotheses carry
substantive content.

\subsection{Parser Independence}

The proposition we want is not merely a type-checking observation. We need a
...
make this precise with the following proposition, which has two halves -- a
syntactic well-typedness half, and a semantic equality half.

\begin{proposition}[Parser Independence]
...
Both halves follow directly from the definitions. Half (I) holds because the
parser's domain is $\Bstar$ by Definition~\ref{def:source}, independent of
which $A$ or $\pi$ produced the bytes. Half (II) holds because a
deterministic parser is a function of its input alone: if the two navigation
paths return the same bytes, the parser returns the same record. The
proposition's value is not mathematical novelty -- it is naming the principle
so that it can be empirically tested (Section~\ref{sec:case3}) and
mechanically enforced (Section~\ref{sec:implementation}).

The proposition requires the parser to be deterministic and side-effect-free:
no hash-randomized internal maps, no system-clock-dependent fields, no calls
to external schema-resolution services. We discuss empirical compliance as a
threat-to-validity (Section~\ref{sec:discussion}).

\subsection{Layer Dependency Rule}

The dependency rule is best understood as the contrapositive of
Proposition~\ref{thm:parser-indep}. We state it correspondingly. The
structure is: \emph{if Parser Independence is required of a parser, then
the parser must not depend on NAVIGATE.}

\begin{proposition}[Layer Dependency Rule]
...
required to satisfy Parser Independence (Proposition~\ref{thm:parser-indep})
...
The rule is the practitioner's read of Proposition~\ref{thm:parser-indep}:
a parser that imports a navigation crate has domain $A_\pi$, not $\Bstar$,
so the composition $p \circ f'_{\mathrm{nav}}$ for any $\pi' \neq \pi$ is
ill-typed. Parser Independence fails not by coincidence but by construction.
In Issen, this is enforced by crate boundaries and build-system rejection
of cyclic or upward dependencies (Section~\ref{sec:implementation}).
...
\begin{proposition}[Partial Completeness over Locally-Computable Oracles]
...
The five oracle disciplines are constructed to match the address algebra of
each primitive (Definition~\ref{def:addr-algebra}): oracle (i) corresponds
to $A_P$, (ii) to $A_M$, (iii) to $A_L$, (iv) to $A_Q$, (v) to $A_C$.
A navigation function using multiple oracles reduces to its primary oracle
under the address-space equivalence $\sim$.
...
Proposition~\ref{thm:partial-complete} is over deterministic Turing machines
