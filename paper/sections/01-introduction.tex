...
fundamentally different ways of locating bytes. The address space dictates
what dependency a parser must take on its source, what trust an investigator
can place in the captured evidence, and whether two investigations can be
joined.

This paper develops the consequences of taking the address space seriously.
We propose a taxonomy of five \emph{navigation primitives} -- the abstract
addressing scheme of an evidence source -- and show that classification by
navigation primitive yields properties that artifact-type classification
cannot. We formalize evidence sources, prove that parsers can be made
source-agnostic, characterize attacker-durability across the primitives, and
show how the taxonomy translates directly into a layered software
architecture. We instantiate the framework in \emph{Issen}, a Rust
orchestration platform built around thirteen crates whose layer membership
is derived mechanically from the taxonomy.

Our position is not that artifact-type classification is wrong -- it is
necessary for analyst-facing presentation -- but that it is insufficient as
the foundation of forensic infrastructure. The two views are complementary:
one tells the analyst what they are looking at; the other tells the engineer
how to build something that can look at it from anywhere.
...
\begin{enumerate}
    ...
    A Parser Independence proposition: identical bytes yield identical
    records under a fixed deterministic parser, regardless of the navigation
    primitive that retrieved the bytes. The result is structural -- it
    follows from the type-theoretic definitions -- but converts a folk
    design principle into an explicitly named, empirically falsifiable claim
    ...
    The PARSER-NAVIGATE layer dependency rule, derived as the contrapositive
    of Proposition~\ref{thm:parser-indep}; the rule converts a folk software
    discipline into an explicitly-stated architectural constraint and we
    instantiate it in Issen's crate boundaries, enforced mechanically by the
    build system.
...
reviews existing process and evidence models and motivates the gap.
Section~\ref{sec:taxonomy} develops the formal taxonomy.
Section~\ref{sec:properties} establishes the central structural propositions.
Section~\ref{sec:implementation} describes the Issen reference
implementation. Section~\ref{sec:cases} presents the case studies.
Section~\ref{sec:related} situates the work against prior literature.
Section~\ref{sec:discussion} addresses completeness and threats to validity.
